...
export class RelayService {
  ...
  async pollLoop(){
    ...
    while(this.running){
      ...
      try{await this.register();const r=await postJson(this.activeRelay,'/p2p/relay/poll',this.identity.envelope({networkId:this.c.networkId}),8000);failures=0;for(const m of r.messages||[]){let result=null,error=null;try{if(m.kind==='infer')result=await this.onInfer(m.payload);else if(m.kind==='validate')result=await this.onValidate(m.payload);else if(m.kind==='commit')result=await this.onCommit(m.payload);else if(m.kind==='transaction')result=await this.onTransaction(m.payload);else if(m.kind==='evolution')result=await this.onEvolution(m.payload,m.source);else if(m.kind==='learning')result=await this.onLearning(m.payload,m.source);else if(m.kind==='evolution-anchor-validate')result={vote:await this.onEvolutionAnchorValidate(m.payload,m.source)};else if(m.kind==='telemetry')result=await this.onTelemetry(m.payload,m.source);else throw new Error('unsupported relay kind');}catch(e){error=e.message;}await postJson(this.activeRelay,'/p2p/relay/respond',this.identity.envelope({networkId:this.c.networkId,requestId:m.requestId,result,error}),8000);}}
