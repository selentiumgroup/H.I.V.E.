...
const c=loadConfig();
let identity,wallet;
try{identity=new Identity(c.dataDir,{password:c.nodeKeyPassword,fallbackPasswords:c.legacyNodeKeyPasswords,allowInsecure:c.allowInsecureKeystore});}
catch(e){if(String(e?.message||'').includes('invalid keystore password'))throw new Error('node identity keystore cannot be decrypted. Restore the previous data/.local-keystore-secret or set LEGACY_NODE_KEY_PASSWORD / LEGACY_WALLET_PASSWORD to the password used before the upgrade. Do not delete identity-v2.json.');throw e;}
try{wallet=new Wallet(c.dataDir,{password:c.walletPassword,fallbackPasswords:c.legacyWalletPasswords,allowInsecure:c.allowInsecureKeystore});}
catch(e){if(String(e?.message||'').includes('invalid keystore password'))throw new Error('NRN wallet keystore cannot be decrypted. Restore the previous data/.local-keystore-secret or set LEGACY_WALLET_PASSWORD to the password used before the upgrade. Do not delete wallet-v2.json.');throw e;}
